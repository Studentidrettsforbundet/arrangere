...
import { setStatusToSubmitted } from "../components/inputFields/setStatusToSubmitted";
 ...
 describe("Test", () => {
    ...
    it("Test if status field gets updated to submitted on doc", async () => {
        ...
        await setStatusToSubmitted(docRef, "SqHIu7oPUeaQKU19ev2UFpjXMsv2", "ot8ttU6nGW7mtpltrZGZ")

    let fieldPath = `status`;
    let value = await firestore.collection("snmApplications").doc("ot8ttU6nGW7mtpltrZGZ").get()
    .then((res: any) => {

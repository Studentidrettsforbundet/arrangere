...
import {
  ...
  addDocToUser,
} from "./components/inputFields/addDocToUser";
...
describe("", () => {
  ...
  it("Test if doc is added to user", async () => {
    await addDocToUser(
      "y0Z97CZIC0NPEz2QbUl3BoXzvPZz",
      "newDocIDI7OUcA12yWZ5",
      "snm"
    );
    const counter = await getNumberOfApplications(
      "y0Z97CZIC0NPEz2QbUl3BoXzvPZz"
    );
    ...
    await firestore
      .collection("user")
      .doc("y0Z97CZIC0NPEz2QbUl3BoXzvPZz")
      .set(
        {
          applications: {
            ["application2"]: firebase.firestore.FieldValue.delete(),
          },
        },
        { merge: true }
      )
